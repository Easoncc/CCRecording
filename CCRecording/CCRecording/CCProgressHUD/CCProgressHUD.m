...
#ifndef kCFCoreFoundationVersionNuCCer_iOS_7_0
...
#define CCMainThreadAssert() NSAssert([NSThread isMainThread], @"CCProgressHUD needs to be accessed on the main thread.");

CGFloat const CCProgressMaxOffset = 1000000.f;

static const CGFloat CCDefaultPadding = 4.f;
static const CGFloat CCDefaultLabelFontSize = 16.f;
static const CGFloat CCDefaultDetailsLabelFontSize = 12.f;


@interface CCProgressHUD () {
    // Deprecated
...
@interface CCProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    CCProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    CCProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (CCProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (CCProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = CCProgressHUDAnimationFade;
    _mode = CCProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(CCProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == CCProgressHUDAnimationZoom) {
        type = animatingIn ? CCProgressHUDAnimationZoomIn : CCProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == CCProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == CCProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == CCProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == CCProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    CCProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<CCProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    CCBackgroundView *backgroundView = [[CCBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = CCProgressHUDBackgroundStyleSolidColor;
    ...
    CCBackgroundView *bezelView = [CCBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:CCDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:CCDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [CCProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:CCDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[CCRoundProgressView class]];

    CCProgressHUDMode mode = self.mode;
    if (mode == CCProgressHUDModeIndeterminate) {
    ...
    else if (mode == CCProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[CCBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == CCProgressHUDModeDeterminate || mode == CCProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[CCRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == CCProgressHUDModeAnnularDeterminate) {
            [(CCRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == CCProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == CCProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[CCProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[CCProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[CCRoundProgressView class]]) {
        CCRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [CCRoundProgressView appearanceWhenContainedIn:[CCProgressHUD class], nil];
#else
        appearance = [CCRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[CCProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((CCRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((CCRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[CCBarProgressView class]]) {
        CCBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [CCBarProgressView appearanceWhenContainedIn:[CCProgressHUD class], nil];
#else
        appearance = [CCBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[CCProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((CCBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((CCBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    CCBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? CCDefaultPadding : 0.f;
...
- (void)setMode:(CCProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == CCProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation CCRoundProgressView
...
@implementation CCBarProgressView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = CCProgressHUDBackgroundStyleBlur;
            ...
            _style = CCProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(CCProgressHUDBackgroundStyle)style {
    if (style == CCProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = CCProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    CCProgressHUDBackgroundStyle style = self.style;
    if (style == CCProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == CCProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation CCProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [CCProgressHUD allHUDsForView:view];
    for (CCProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable CCProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    CCBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == CCProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    CCMainThreadAssert();
    self.backgroundView.style = CCProgressHUDBackgroundStyleSolidColor;
...
@implementation CCProgressHUDRoundedButton
